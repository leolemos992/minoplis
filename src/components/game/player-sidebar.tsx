...
// Componente para um único jogador, para isolar a busca de dados
function PlayerAccordionItem({ player }: { player: Player }) {
    const firestore = useFirestore();
    const userProfileRef = useMemoFirebase(() => firestore ? doc(firestore, 'users', player.userId) : null, [firestore, player.userId]);
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 text-left">
                        <p className="font-semibold flex items-center gap-2">
                            {player.name}
                            {profile && <span className="flex items-center text-xs text-yellow-400"><Star className="w-3 h-3 mr-1" /> Nv. {profile.level}</span>}
                        </p>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <Accordion type="single" collapsible defaultValue={loggedInPlayerId} className="w-full mt-2">
            {allPlayers.map(player => (
                <PlayerAccordionItem key={player.id} player={player} />
            ))}
        </Accordion>
      </div>
